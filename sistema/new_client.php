<?php 
	...
	if (!empty($_POST)) {
		...
		if (empty($_POST['cedularif']) || empty($_POST['nombre']) || empty($_POST['correo']) || empty($_POST['telefono']) || empty($_POST['direccion'])) {
            ...
            if (is_numeric($cedularif)) {
                $query = mysqli_query($conection,"SELECT * FROM cliente WHERE cedularif = '$cedularif'");
            ...
            if ($result > 0) {
                ...
                $query_insert = mysqli_query($conection,"INSERT INTO cliente(cedularif,nombre,correo,telefono,direccion,id_usuario) 
                                                            VALUES('$cedularif',  '$nombre', '$email', '$telefono',  '$direccion', '$id_usuario')");
